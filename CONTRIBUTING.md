...
- **&lt;user&gt;:** Your GitHub/Linear username (lowercase). Example: `yourname`, `jdoe`.
- **&lt;IDENTIFIER&gt;:** Full Linear issue ID (e.g. `AIT-12`, `AIT-266`). **Required.** Case-insensitive (`AIT` or `ait` both work).
- **&lt;short-slug&gt;:** Short kebab-case description. Example: `fix-login`, `add-linear-check`.
...
> Branch must follow format user/AIT-123-description to auto-link Linear tickets.

### Using the Cursor rule (automatic)

The Cursor rule **`.cursor/rules/linear-workflow.mdc`** automates kickoff and branching:
...
2. Create a branch locally: `git checkout -b <your-username>/AIT-<number>-<short-slug>`.
