#!/bin/bash
# todo - AI-Agent First TODO List Tracker (Bash Version)
# 
...
EOF
    ...
    if [[ "$key" =~ ^([^.]+)\.(.+)$ ]]; then
        local parent_key="${match[1]}"
        local child_key="${match[2]}"
...
collect_task_notes() {
    ...
    local task_line_num=$(grep -n "^- \[.\] \*\*#${escaped_task_id}\*\* \|^  - \[.\] \*\*#${escaped_task_id}\*\* \|^    - \[.\] \*\*#${escaped_task_id}\*\* " "$TODO_FILE" | head -1 | cut -d: -f1)
    ...
    if [[ -n "$task_line_num" ]]; then
        ...
        while true; do
            ...
            # Check for blockquotes at different indentation levels
...
show_task() {
    ...
    local task_line=$(grep "^- \[.\] \*\*#$resolved_task_id\*\* \|^  - \[.\] \*\*#$resolved_task_id\*\* " "$TODO_FILE" | head -1)
    ...
    if [[ -z "$task_line" ]] && [[ "$resolved_task_id" != "$task_id" ]] && [[ "$resolved_task_id" =~ ^[a-z0-9]{1,7}-([0-9]+(\.[0-9]+)?)$ ]]; then
        ...
        task_line=$(grep "^- \[.\] \*\*#${plain_number}\*\* \|^  - \[.\] \*\*#${plain_number}\*\* " "$TODO_FILE" | head -1)
        if [[ -n "$task_line" ]]; then
...
add_note() {
    ...
    # Verify task exists (check main task, subtask, and sub-subtask patterns)
    local task_line_num=$(grep -n "^- \[.\] \*\*#$task_id\*\* \|^  - \[.\] \*\*#$task_id\*\* \|^    - \[.\] \*\*#$task_id\*\* " "$TODO_FILE" | head -1 | cut -d: -f1)
    ...
    if [[ "$task_line" =~ ^"    - " ]]; then
        # This is a sub-subtask (4 spaces), needs 4 spaces indentation
        indent="    "
    elif [[ "$task_line" =~ ^"  - " ]]; then
        # This is a subtask (2 spaces), needs 2 spaces indentation
        indent="  "
    fi
...
create_new_issue() {
    ...
    if gh issue create --repo "$repo_url" --title "Bug: $title" --body-file "$temp_body" --label "bug,auto-reported" >/dev/null 2>&1; then
        ...
        echo "✅ Bug report created successfully (without auto-reported label)"
        rm -f "$temp_body"
        return 0
    else
