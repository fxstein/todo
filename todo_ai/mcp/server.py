        ...
        async def call_tool(name: str, arguments: dict) -> list[types.TextContent]:
            ...
            try:
                if name == "add_task":
                    ...
                    task = manager.add_task(description=arguments["description"], tags=tags_list)
                    self.file_ops.write_tasks(manager.list_tasks())
                    return [
                        types.TextContent(type="text", text=f"Added: #{task.id} {task.description}")
                    ]
                    ...
                    return [
                        types.TextContent(
                            type="text", text=f"Completed: #{task.id} {task.description}"
                        )
                    ]
