...
# Parse global --root override before initialization
ROOT_OVERRIDE_PATH=""
ROOT_FILTERED_ARGS=()
ROOT_EXPECTING_VALUE="false"
for arg in "$@"; do
    if [[ "$ROOT_EXPECTING_VALUE" == "true" ]]; then
        ROOT_OVERRIDE_PATH="$arg"
        ROOT_EXPECTING_VALUE="false"
        continue
    fi
    case "$arg" in
        --root)
            ROOT_EXPECTING_VALUE="true"
            ;;
        --root=*)
            ROOT_OVERRIDE_PATH="${arg#--root=}"
            ;;
        *)
            ROOT_FILTERED_ARGS+=("$arg")
            ;;
    esac
done
if [[ "$ROOT_EXPECTING_VALUE" == "true" ]]; then
...
if (( ${#ROOT_FILTERED_ARGS[@]} > 0 )); then
    set -- "${ROOT_FILTERED_ARGS[@]}"
else
    set --
fi
...
resolve_abs_path() {
    ...
    if [[ "$raw_path" != /* ]]; then
        raw_path="${ORIGINAL_WORKING_DIR}/${raw_path}"
    fi
    (cd "$raw_path" 2>/dev/null && pwd -P)
}

resolve_git_root() {
    if command -v git >/dev/null 2>&1 && git -C "$ORIGINAL_WORKING_DIR" rev-parse --git-dir >/dev/null 2>&1; then
        local super_root
        super_root=$(git -C "$ORIGINAL_WORKING_DIR" rev-parse --show-superproject-working-tree 2>/dev/null || true)
...
if [[ -n "$ROOT_OVERRIDE_PATH" ]]; then
    if resolved_root="$(resolve_abs_path "$ROOT_OVERRIDE_PATH")"; then
        ROOT_DIR="$resolved_root"
        ROOT_SOURCE="--root"
    else
        echo "Warning: --root path not found: $ROOT_OVERRIDE_PATH" >&2
    fi
fi
if [[ -z "$ROOT_DIR" && -n "${TODO_AI_ROOT:-}" ]]; then
    if resolved_root="$(resolve_abs_path "$TODO_AI_ROOT")"; then
        ROOT_DIR="$resolved_root"
        ROOT_SOURCE="env"
    else
        echo "Warning: TODO_AI_ROOT path not found: $TODO_AI_ROOT" >&2
    fi
fi
if [[ -z "$ROOT_DIR" && -n "$git_root" ]]; then
    ROOT_DIR="$git_root"
    ROOT_SOURCE="git"
fi
if [[ -z "$ROOT_DIR" ]]; then
    ROOT_DIR="$ORIGINAL_WORKING_DIR"
    ROOT_SOURCE="cwd"
fi

TODO_FILE="${TODO_FILE:-${ROOT_DIR}/TODO.md}"
SERIAL_FILE="${TODO_SERIAL:-${ROOT_DIR}/.todo.ai/.todo.ai.serial}"
LOG_FILE="${TODO_LOG:-${ROOT_DIR}/.todo.ai/.todo.ai.log}"
CONFIG_FILE="${TODO_CONFIG:-${ROOT_DIR}/.todo.ai/config.yaml}"
...
create_mode_backup() {
    local backup_dir="${ROOT_DIR}/.todo.ai/backups"
...
EOF
    ...
    if [[ "$renumber" == false ]]; then
        echo "📝 Note: Existing tasks keep their current IDs"
        echo "   New tasks will use the $new_mode numbering scheme"
        echo "   Use --renumber flag to renumber existing tasks"
    else
        echo "📝 All tasks have been renumbered to match $new_mode numbering scheme"
    fi
    echo ""
...
**Examples:**
- ✅ User says \"report a bug about X\": Use \`$todo_ai_path report-bug \"X\" \"context\" \"command\"\`
- ✅ \"An error occurred. Would you like me to report this bug?\" → If yes, use \`$todo_ai_path report-bug\`
...
**Examples:**
- ✅ User says \"uninstall todo.ai\": Use \`$todo_ai_path uninstall\`
- ✅ User says \"remove todo.ai and data\": Use \`$todo_ai_path uninstall --remove-data\`
- ✅ User says \"delete todo.ai completely\": Use \`$todo_ai_path uninstall --all\`
- ❌ \"Removing todo.ai files...\" and using \`rm todo.ai\` (NEVER DO THIS)
- ❌ Using \`rm -rf .todo.ai\` directly (NEVER DO THIS)
- ❌ Deleting .cursor/rules/todo.ai-*.mdc files manually (NEVER DO THIS)
...
**Examples:**
- ✅ \`$todo_ai_path note 42 \"Modify archive_task() lines 2600-2850. Pattern from delete_task() lines 2926-2935\"\`
- ✅ \`$todo_ai_path note 42.1 \"Depends on task#42 completing first - needs collect_notes() function\"\`
- ✅ \`$todo_ai_path note 50 \"See docs/CODE_SIZE_ANALYSIS.md lines 66-67 for migration cleanup details\"\`
...
EOF
    ...
    if [[ $rules_created -gt 0 ]] || [[ $rules_updated -gt 0 ]]; then
        echo ""
        echo "⚠️  IMPORTANT: Cursor rules initialized"
        if [[ $rules_created -gt 0 ]]; then
            echo "   📝 Created $rules_created rule file(s) in .cursor/rules/"
        fi
        if [[ $rules_updated -gt 0 ]]; then
            echo "   📝 Updated $rules_updated rule file(s) in .cursor/rules/"
        fi
        echo "   This ensures AI agents will automatically use todo.ai for task tracking"
        echo ""
        echo "   🔄 ACTION REQUIRED: Cursor rules have been initialized!"
        echo ""
        echo "   For Humans:"
        echo "   → Please start a new chat session in Cursor"
        echo "   → Cursor only loads rules at the start of each chat session"
        echo "   → The rules will not apply to your current session"
        echo ""
        echo "   For AI Agents:"
        echo "   → Inform the user that cursor rules have been initialized"
...
show_usage() {
    ...
    echo "Global options:"
    echo "  --root <path>                 Override repo root for this invocation"
    echo ""
    ...
    echo "  show-root                     Show resolved root and source"
...
show_root() {
    echo "Resolved root: $ROOT_DIR (source: $ROOT_SOURCE)"
    if [[ "$ROOT_SOURCE" == "env" ]]; then
        echo "Override: TODO_AI_ROOT=$TODO_AI_ROOT"
    elif [[ "$ROOT_SOURCE" == "--root" ]]; then
        echo "Override: --root $ROOT_OVERRIDE_PATH"
    fi
}
...
delete_task() {
    ...
    for number in "${all_ids[@]}"; do
        ...
        if [[ -z "$task_line" ]] && [[ "$number" =~ ^[a-z0-9]{1,7}-([0-9]+(\.[0-9]+)?)$ ]]; then
            ...
            local escaped_plain=$(echo "$number_without_prefix" | sed 's/\./\\./g')
            task_line=$(grep -E "^- \[.\] (\*\*#${escaped_plain}\*\*|#${escaped_plain}) |^  - \[.\] (\*\*#${escaped_plain}\*\*|#${escaped_plain}) |^    - \[.\] (\*\*#${escaped_plain}\*\*|#${escaped_plain}) " "$TODO_FILE" | head -1)
            if [[ -n "$task_line" ]]; then
...
detect_coordination_options() {
    ...
    if command -v git >/dev/null 2>&1 && git rev-parse --git-dir >/dev/null 2>&1; then
        ...
        if [[ -n "$repo_url" ]] && [[ "$repo_url" =~ github\.com ]]; then
            git_repo=true
            echo "  ✅ Git repository with GitHub remote detected"
        else
            echo "  ⚠️  Git repository not connected to GitHub"
    ...
    if command -v jq >/dev/null 2>&1; then
        ...
        echo "  ✅ python3 installed (can parse JSON)"
    else
    ...
    if [[ ${#available_options[@]} -eq 0 ]]; then
        echo "  ❌ No coordination options available"
        echo ""
        ...
        echo ""
        echo "💡 Recommendation: Use 'multi-user' or 'branch' mode (no coordination required)"
        return 1
    else
        ...
        if [[ ${#missing_requirements[@]} -gt 0 ]]; then
            echo "⚠️  Note: Some options may require additional setup"
        fi
        return 0
    fi
}
...
setup_github_issues_coordination() {
    ...
    if [[ -n "$existing_issues" ]]; then
        ...
        echo "✅ Found existing coordination issue #$issue_number"
    else
...
Do not manually comment on this issue - it is managed automatically by todo.ai."
    ...
    if [[ "$existing_comments" == "0" ]] || [[ -z "$existing_comments" ]]; then
        ...
        if ! gh api -X POST repos/${repo_identifier}/issues/${issue_number}/comments --field "body=Next task number: $start_num" >/dev/null 2>&1; then
            ...
            echo "✅ Initialized issue with starting number: $start_num"
        fi
    else
        ...
        if [[ -n "$current_num" ]]; then
            ...
            if [[ $current_num -lt $highest_num ]]; then
                ...
                if gh api -X POST repos/${repo_identifier}/issues/${issue_number}/comments --field "body=Next task number: $sync_num" >/dev/null 2>&1; then
                    ...
                    echo "   ⚠️  Warning: Failed to sync coordinator"
                fi
            fi
        fi
    fi
...
setup_counterapi_coordination() {
    ...
    if [[ "$interactive" == true ]]; then
        ...
        if [[ -z "$namespace" ]]; then
            echo "❌ Error: COUNTERAPI_NAMESPACE environment variable not set"
            return 1
        fi
    fi
...
EOF
        if [[ $? -ne 0 ]]; then
            ...
            echo "⚠️  Python YAML update failed, using sed fallback..."
        else
        ...
        if [[ ${#available[@]} -eq 0 ]]; then
            ...
            if [[ $coord_choice -ge 1 ]] && [[ $coord_choice -le ${#available[@]} ]]; then
                ...
                setup_coordination "$selected_coord" || {
                    echo "⚠️  Coordination setup failed, continuing with fallback mode"
                }
            else
...
resolve_conflicts() {
    ...
    if [[ "$dry_run" == true ]]; then
        ...
        for key in "${!id_mapping[@]}"; do
            ...
            if [[ "$line_num" =~ ^[0-9]+$ ]]; then
                ...
                echo "  Line $line_num: #$old_id → #$new_id"
                echo "    $task_line"
            else
                echo "  ⚠️  Invalid line number format: '$line_num' (key: '$key')"
            fi
        done
        echo ""
        ...
        while IFS= read -r line; do
            ...
            for key in "${!id_mapping[@]}"; do
                ...
                if [[ "$dup_line_num" =~ ^[0-9]+$ ]] && [[ "$line_num" == "$dup_line_num" ]]; then
                    ...
                    echo "  ✅ Renumbered: Line $line_num: #$old_id → #$new_id"
                    break
                fi
            done
    ...
    if [[ "$dry_run" == true ]]; then
        ...
        echo "✅ Resolved $conflicts_resolved conflict(s)"
    else
...
init_backups_dir() {
    local backups_dir="${ROOT_DIR}/.todo.ai/backups"
...
get_script_path() {
    ...
    elif [[ -f "${ROOT_DIR}/todo.ai" ]]; then
        script_path="${ROOT_DIR}/todo.ai"
...
update_tool() {
    ...
    if curl -s -o "$temp_file" "$SCRIPT_URL"; then
        ...
        if [[ -s "$temp_file" ]] && grep -q "todo - AI-Agent First TODO List Tracker" "$temp_file"; then
            ...
            echo ""
            echo "🔄 Executing new version's update logic (v$new_version)..."
            local current_dir=$(pwd)
        ...
        if version_compare "$current_version" "$target_version" ">="; then
            ...
            if [[ ! -f "$migration_file" ]]; then
                ...
                if "$function_name"; then
                    ...
                    echo "⚠️  Migration failed: $description" >&2
                fi
            fi
        fi
    done
...
<details>
<summary><strong>📜 Recent Commands</strong> (last 5)</summary>
...
create_new_issue() {
    ...
    if gh issue create --repo "$repo_url" --title "$title" --body-file "$temp_body" --label "$labels" 2>&1 | tee /tmp/gh-output; then
        ...
        echo "⚠️  Some labels not available, retrying with bug label only..."
        if gh issue create --repo "$repo_url" --title "$title" --body-file "$temp_body" --label "bug" >/dev/null 2>&1; then
            echo "✅ Bug report created successfully (with basic labels)"
            rm -f "$temp_body"
            return 0
        else
...
uninstall_tool() {
    ...
    if [[ "$remove_data" == true ]] && [[ "$has_data_dir" == true ]]; then
        ...
        echo "     ⚠️  Warning: This will remove your TODO data (.todo.ai/ directory)"
        echo "     ⚠️  Your TODO.md file will remain untouched"
    ...
    if [[ "$will_remove_script" == true ]]; then
        if rm -f "$script_path" 2>/dev/null; then
            ...
            echo "⚠️  Error: Could not remove script: $script_path"
    ...
    if [[ "$will_remove_data" == true ]]; then
        if rm -rf "$todo_ai_dir" 2>/dev/null; then
            echo "✅ Removed data directory: $todo_ai_dir"
        else
            echo "⚠️  Error: Could not remove data directory: $todo_ai_dir"
            echo "   You may need to remove it manually"
        fi
    fi
    ...
    if [[ "$will_remove_rules" == true ]]; then
        ...
        if [[ $rules_removed -gt 0 ]]; then
            echo "✅ Removed $rules_removed Cursor rule(s)"
        else
            echo "⚠️  Error: Could not remove Cursor rules"
...
if [[ "${1:-}" != "version" && "${1:-}" != "--version" && "${1:-}" != "-v" && \
    ...
    if [[ "$current_mode" == "enhanced" ]] || [[ "$current_mode" == "single-user" ]]; then
        ...
        if [[ "$coord_type" != "none" ]] && [[ -n "$coord_type" ]]; then
            case "$coord_type" in
                "github-issues")
                    issue_num=$(get_config_value "coordination.issue_number" "")
                    if [[ -n "$issue_num" ]]; then
                        echo "📋 Mode: $current_mode | Coordinator: GitHub Issues (#$issue_num)" >&2
                    else
                        echo "📋 Mode: $current_mode | Coordinator: GitHub Issues (not configured)" >&2
                    fi
                    ;;
                "counterapi")
                    namespace=$(get_config_value "coordination.namespace" "")
                    if [[ -n "$namespace" ]]; then
                        echo "📋 Mode: $current_mode | Coordinator: CounterAPI ($namespace)" >&2
                    else
                        echo "📋 Mode: $current_mode | Coordinator: CounterAPI (not configured)" >&2
                    fi
                    ;;
                *)
                    echo "📋 Mode: $current_mode | Coordinator: $coord_type" >&2
                    ;;
            esac
        else
            # No coordination configured
            if [[ "$current_mode" == "enhanced" ]]; then
                echo "📋 Mode: $current_mode | Coordinator: none (fallback)" >&2
            else
