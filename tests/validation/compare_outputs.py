...
def main():
    ...
    (test_dir / "TODO.md").write_text("# todo.ai ToDo List\n\n## Tasks\n\n------------------\n**Maintenance:** Use `todo.ai` script only\n", encoding="utf-8")
    ...
    py_lines = [l.strip() for l in py_norm.splitlines() if l.strip()]
    sh_lines = [l.strip() for l in sh_norm.splitlines() if l.strip()]
    
    match = True
    if len(py_lines) != len(sh_lines):
        match = False
    else:
        for p, s in zip(py_lines, sh_lines):
            if p != s:
                match = False
                break
